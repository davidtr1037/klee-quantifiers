...
ref<Expr> Executor::rewriteSwitchCaseCondition(ExecutionState &state,
  ...
  for (ref<ConstantExpr> c : constants) {
    if (!intervals.empty()) {
      std::vector<ref<ConstantExpr>> &interval = intervals.back();
      assert(!interval.empty());
      ref<ConstantExpr> max = interval.back();
      if (c->getZExtValue() == max->getZExtValue() + 1) {
        interval.push_back(c);
        continue;
      }
    }
    std::vector<ref<ConstantExpr>> interval = {c};
    intervals.push_back(interval);
  }

  ref<Expr> condition = ConstantExpr::create(0, Expr::Bool);
  for (std::vector<ref<ConstantExpr>> &interval : intervals) {
    ref<Expr> intervalCondition;
    if (interval.size() == 1) {
      ref<ConstantExpr> c = interval.front();
      intervalCondition = EqExpr::create(value, c);
    } else {
      ref<ConstantExpr> min = interval.front();
      ref<ConstantExpr> max = interval.back();
      intervalCondition = AndExpr::create(
        UgeExpr::create(value, min),
        UleExpr::create(value, max)
      );
    }
...
ref<Expr> Executor::getSwitchCaseCondition(ExecutionState &state,
  ...
  for (ref<ConstantExpr> c : constants) {
    ...
    if (mayBeTrue) {
      ref<Expr> match = EqExpr::create(value, c);
      condition = OrExpr::create(match, condition);
    }
  }
...
void Executor::getSwitchForks(ExecutionState &state,
                              KInstruction *ki,
                              std::vector<BasicBlock *> &blocks,
                              std::map<BasicBlock *, ref<Expr>> &caseConditions) {
  SwitchInst *si = cast<SwitchInst>(ki->inst);
  ref<Expr> value = eval(ki, 0, state).value;

  std::map<ref<ConstantExpr>, BasicBlock *> expr2bb;
  std::map<BasicBlock *, std::vector<ref<ConstantExpr>>> bb2constants;
  for (SwitchInst::CaseHandle caseHandle : si->cases()) {
    ref<ConstantExpr> c = evalConstant(caseHandle.getCaseValue());
    BasicBlock *caseSuccessor = caseHandle.getCaseSuccessor();
    expr2bb.insert(std::make_pair(c, caseSuccessor));
    bb2constants[caseSuccessor].push_back(c);
  }
  ...
  for (auto &i : expr2bb) {
    ...
    if (std::find(blocks.begin(), blocks.end(), caseSuccessor) != blocks.end()) {
      continue;
    }
    ...
    if (!caseCondition->isFalse()) {
      ...
      defaultCondition = AndExpr::create(defaultCondition, Expr::createIsZero(caseCondition));
      blocks.push_back(caseSuccessor);
    }
  }

  defaultCondition = optimizer.optimizeExpr(defaultCondition, false);
  bool mayBeTrue;
  bool success = solver->mayBeTrue(&state,
                                   state.constraints,
                                   defaultCondition,
  ...
  if (mayBeTrue) {
    auto res = caseConditions.insert(std::make_pair(si->getDefaultDest(), defaultCondition));
    if (res.second) {
      blocks.push_back(si->getDefaultDest());
    }
  }
}
...
Executor::fork(ExecutionState &current, ref<Expr> condition, bool isInternal) {
  ...
  if (!isSeeding && !isa<ConstantExpr>(condition) && 
    ...
    if ((MaxStaticForkPct<1. &&
      ...
      bool success = solver->getValue(&current,
                                      current.constraints, condition, value,
  ...
  bool success = solver->evaluate(&current,
                                  current.constraints, condition, res,
                                  current.queryMetaData);
...
ref<Expr> Executor::toUnique(const ExecutionState &state, 
  ...
  if (!isa<ConstantExpr>(e)) {
    ...
    if (solver->getValue(&state, state.constraints, e, value, state.queryMetaData)) {
      ref<Expr> cond = EqExpr::create(e, value);
      cond = optimizer.optimizeExpr(cond, false);
      if (solver->mustBeTrue(&state,
                             state.constraints, cond, isTrue,
                             state.queryMetaData) &&
          isTrue)
        result = value;
    }
...
Executor::toConstant(ExecutionState &state, 
  ...
  ref<ConstantExpr> value;
  bool success =
      solver->getValue(&state, state.constraints, e, value, state.queryMetaData);
  assert(success && "FIXME: Unhandled solver failure");
  (void) success;
...
void Executor::executeGetValue(ExecutionState &state,
  ...
  if (it==seedMap.end() || isa<ConstantExpr>(e)) {
    ...
    for (std::vector<SeedInfo>::iterator siit = it->second.begin(), 
      ...
      ref<ConstantExpr> value;
      bool success =
          solver->getValue(&state, state.constraints, cond, value, state.queryMetaData);
      assert(success && "FIXME: Unhandled solver failure");
      (void) success;
      values.insert(value);
    }
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::IndirectBr: {
    ...
    bool success __attribute__((unused)) = solver->mayBeTrue(
        &state, state.constraints, errorCase, result, state.queryMetaData);
    assert(success && "FIXME: Unhandled solver failure");
    if (result) {
  ...
  case Instruction::Switch: {
    ...
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(cond)) {
      ...
      getSwitchForks(state, ki, bbOrder, branchTargets);
  ...
  case Instruction::Call: {
    ...
    if (f) {
      ...
      do {
        ...
        ref<ConstantExpr> value;
        bool success =
            solver->getValue(free, free->constraints, v, value, free->queryMetaData);
        assert(success && "FIXME: Unhandled solver failure");
        (void) success;
...
std::string Executor::getAddressInfo(ExecutionState &state, 
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(address)) {
    ...
    bool success = solver->getValue(&state,
                                    state.constraints, address, value,
                                    state.queryMetaData);
    assert(success && "FIXME: Unhandled solver failure");
    (void) success;
...
void Executor::terminateState(ExecutionState &state) {
  ...
  if (!state.loopHandler.isNull()) {
    if (state.stack.back().isExecutingLoop && !haltExecution) {
      /* a state that did't reach a loop exit */
  ...
  for (std::vector<ref<Expr> >::iterator ai = arguments.begin(), 
    ...
    if (ExternalCalls == ExternalCallPolicy::All) { // don't bother checking uniqueness
      *ai = optimizer.optimizeExpr(*ai, true);
      ref<ConstantExpr> ce;
      bool success =
          solver->getValue(&state, state.constraints, *ai, ce, state.queryMetaData);
      assert(success && "FIXME: Unhandled solver failure");
      (void) success;
...
void Executor::executeAlloc(ExecutionState &state,
  ...
  if (isa<ConstantExpr>(size) || AllocateSymSize) {
    ...
    bool success =
        solver->getValue(&state, state.constraints, size, example, state.queryMetaData);
    assert(success && "FIXME: Unhandled solver failure");
    (void) success;
    ...
    while (example->Ugt(ConstantExpr::alloc(128, W))->isTrue()) {
      ...
      bool res;
      bool success =
          solver->mayBeTrue(&state,
                            state.constraints, EqExpr::create(tmp, size), res,
                            state.queryMetaData);
      assert(success && "FIXME: Unhandled solver failure");      
      (void) success;
      if (!res)
    ...
    if (fixedSize.second) { 
      ...
      bool success = solver->getValue(fixedSize.second,
                                      fixedSize.second->constraints, size, tmp,
                                      fixedSize.second->queryMetaData);
      assert(success && "FIXME: Unhandled solver failure");      
      (void) success;
      bool res;
      success = solver->mustBeTrue(fixedSize.second,
                                   fixedSize.second->constraints,
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  if (success) {
    ...
    bool success = solver->mustBeTrue(&state,
                                      state.constraints, check, inBounds,
                                      state.queryMetaData);
    solver->setTimeout(time::Span());
    if (!success) {
      state.pc = state.prevPC;
...
bool Executor::getSymbolicSolution(const ExecutionState &state,
    ...
    for (; pi != pie; ++pi) {
      ...
      bool success =
          solver->mustBeTrue(&state,
                             extendedConstraints, Expr::createIsZero(*pi),
  ...
  bool success = solver->getInitialValues(&state,
                                          extendedConstraints, objects, values,
                                          state.queryMetaData);
  solver->setTimeout(time::Span());
  if (!success) {
...
size_t Executor::getCapacity(ExecutionState &state, ref<Expr> size) {
  ...
  while (result == Solver::False) {
    ...
    bool success = solver->evaluate(&state, state.constraints, bound, result, state.queryMetaData);
    solver->setTimeout(time::Span());
    if (!success) {
      /* TODO: handle... */
      assert(0);
...
uint64_t Executor::getMaxConcreteValue(ExecutionState &state,
  ...
  for (uint64_t i = 0; i < capacity + 1; i++) {
    ...
    assert(solver->evaluate(&state, state.constraints, eq, result, state.queryMetaData));
    solver->setTimeout(time::Span());
